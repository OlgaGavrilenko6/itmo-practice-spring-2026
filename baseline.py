...
from agent_experiment_req.docx_utils import extract_requirement_fragments_from_docx
...
class RequirementExtractor(BaseOpenAIClient):
    ...
    def extract_requirements_from_fragments(self, fragments: Sequence[str]) -> list[dict]:
        if not fragments:
            return []

        responses: list[Union[ExtractedRequirements, NoneResult]] = self.make_requests(fragments)
        results: list[dict] = []
...
def extract_requirements_from_document(
    ...
    fragments = extract_requirement_fragments_from_docx(
        file_path=file_path,
        invalid_headings=invalid_headings,
        extract_tables=False,
    )

    logger.info("Parsed %s fragments from %s", len(fragments), file_path)
    ...
    logger.info("Extracted requirements from %s fragments", len(results))
    return results
...
if __name__ == "__main__":
    FILE_PATH = "data/СП_251.1325800.2016_с_И1_И2_И3_И4_И5.docx"
    OPENAI_BASE_URL = "http://d.dgx:8071/v1"
    OPENAI_API_TOKEN = "token-abc123"
    MODEL_NAME = "gpt-oss-120b"

    results = extract_requirements_from_document(
        file_path=FILE_PATH,
        openai_base_url=OPENAI_BASE_URL,
        openai_api_token=OPENAI_API_TOKEN,
        model_name=MODEL_NAME,
    ...
    save_grouped_requirements_to_json(results, "requirements_baseline_gpt.json")
